...
class REINFORCE(object):
    ...
    def compute_episode_returns(self,Rs,gamma_w):
        ...
        tm=np.tile(np.arange(0,Rs.shape[0]),(Rs.shape[0],1))
        zf=(np.arange(0,Rs.shape[0])-np.arange(Rs.shape[0]).reshape(-1,1)>=0)
        tm=(gamma_w**tm)*zf
        Rs=np.tile(Rs,(Rs.shape[0],1))
        return (Rs*tm).sum(1)

    def run_init_step(self):
        u=self.free_input
        s=self.env.reset()
    ...
    def run_step(self):
        for ep_step in tqdm(range(self.ep_limit+1)):
            self.episodes_states[self.current_episode].append(self.current_state)
            s=torch.from_numpy(self.current_state).float().unsqueeze(0) #[1,states]
            if self.cuda:
                s=s.cuda()
            pa=self.model.act(s)
...
#
    ...
    def Train(self,train_batches):
        ...
        for batch in tqdm(range(train_batches)):
            ...
            #Compute elegibility as a model method (delta   )
                #Z_w,Z_th,w,th
          ...
          #TODO: for generalization implement compute losses
            ...
            self.episodes_losses[self.current_batch-1]={"loss":batch_loss.detach().cpu().item()}
